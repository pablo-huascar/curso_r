...
Use o código a seguir para baixar um banco de dados de `vendas_semanais`. Em seguida, fransforme o banco de dados `vendas_semanais`, de modo que as colunas que começam com semana se transformem em uma nova variável chamada `semana` e o seus valores vão para uma variável chamada `vendas`. Além disso, transforme os valores da variável `semana` em números. Salve o resultado em um banco de dados chamado `df` 

```{r}
#| eval: false

vendas_semanais <- 
  read_csv("https://tinyurl.com/vendasem")
```
